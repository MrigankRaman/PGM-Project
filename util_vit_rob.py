...
class NLVR2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.model_type == "vilt":
            ...
            input_ids = inputs_dict["input_ids"]
            attention_mask = inputs_dict["attention_mask"]
            return pixel_values_1["pixel_values"].squeeze(0), pixel_values_2["pixel_values"].squeeze(0), label, input_ids.squeeze(0), attention_mask.squeeze(0)
...
eval_data = NLVR2Dataset('/home/mrigankr/PGM/nlvr2/data/dev.json', "vit_rob", "/data/mrigankr/mml/dev/")
train_data = NLVR2Dataset('/home/mrigankr/PGM/nlvr2/data/train.json', "vit_rob", "/data/mrigankr/mml/train/")
...
class ViTRobforNLVR2(nn.Module):
    def __init__(self):
        super(ViTRobforNLVR2, self).__init__()
        self.vit = ViTModel.from_pretrained("google/vit-base-patch16-224-in21k")
        self.rob = AutoModelForSequenceClassification.from_pretrained("roberta-base", num_labels=2, output_hidden_states=True)
...
model = ViTRobforNLVR2()
model.to("cuda")
epochs = 20
train_loader = DataLoader(train_data, batch_size=64, shuffle=True)
eval_loader = DataLoader(eval_data, batch_size=64, shuffle=True)
optimizer = AdamW(model.parameters(), lr=1e-5)
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=0, num_training_steps=len(train_loader) * epochs)
for epoch in range(epochs):
    model.train()
    for batch in tqdm(train_loader):
        pixel_values_1, pixel_values_2, labels, input_ids, attn_ids = batch
        pixel_values_1, pixel_values_2, labels = pixel_values_1.to("cuda"), pixel_values_2.to("cuda"), labels.to("cuda")
        input_ids, attn_ids = input_ids.to("cuda"), attn_ids.to("cuda")
        outputs = model(pixel_values_1, pixel_values_2, input_ids, attn_ids)
    ...
    for batch in tqdm(eval_loader):
        pixel_values_1, pixel_values_2, labels, inputs_ids, attn_ids = batch
        # set to cuda
        pixel_values_1, pixel_values_2, labels = pixel_values_1.to("cuda"), pixel_values_2.to("cuda"), labels.to("cuda")
        inputs_ids, attn_ids = inputs_ids.to("cuda"), attn_ids.to("cuda")
        outputs = model(pixel_values_1, pixel_values_2, inputs_ids, attn_ids)
